...
dataFolder = 'Data/BretonDataTest'
...
for file in velocity_files:
	...
	for cnt, idx in enumerate(sorted_indexes):
		...
		if idx.size >= 20 * 60 * 5:  # 20 mins
			...
			# %%
			deta_torque_wheel = 100000000000
			opt_results_v2 = {}
				...
				if torque >= 0:
					...
					if np.isclose(np.isclose(vel_seq, np.zeros(n_steps_in + n_steps_out)).astype('float32').sum(), 0):
						...
						if np.count_nonzero(vel_pred <= 0) > 0:
							...
							torque_wheel_ctl = torque_seq_ctl[0] * gear_ctl[0] * i0 * eff_diff * eff_cpling if torque_seq_ctl[0] > 0 else torque_seq_ctl[0] * gear_ctl[0] * i0 / (eff_diff * eff_cpling * Reg_rate)
							...
							if flag == 1:
								...
								torque_wheel_dmd = torque_seq_dmd[0] * gear_dmd[0] * i0 * eff_diff * eff_cpling if torque_seq_dmd[0] > 0 else torque_seq_dmd[0] * gear_dmd[0] * i0 / (eff_diff * eff_cpling * Reg_rate)
