...
def choose_gear(vel_current, vel_next, gear_former):
...
########### 这部分用来计算能耗最优的速度轨迹 ########### 
def energy_opt_v3(gears_in_use, gear_former, vel_current, vel_pred, acc=1, dec=-1, gear_pre_duration=3):
	vel_pred = np.insert(vel_pred, 0, vel_current, axis=0)
 ...
 ################################# 选档  #################################   
	vel_next = vel_opt[1]
	gear_opt, flag = choose_gear(vel_current, vel_next, gear_former)
